<?php
    ...
    $sql_httt= "SELECT * FROM hinhthucthanhtoan";
    $stmt_httt = $conn->prepare($sql_httt);
    $query_httt = $stmt_httt->execute();
    $result_httt = array();
    while($row_httt = $stmt_httt->fetch(PDO::FETCH_ASSOC)){
        $result_httt[] = $row_httt;
    }

    $sql_sl = "SELECT count(id) as dem FROM hinhthucthanhtoan";
...
<body>
    <div id="content">
        <div id="admin__sidebar">
         ...
         <div id="main_content">
            <div class="title">
                <h2>Hình thức thanh toán (<?php echo $slusers?>)</h2>
                <a class="btn_themkh" href="hinhthucthanhtoan_themmoi.php">Thêm mới</a>
            </div>
            <div class="chitiet_content">
                <table class="table table-inverse">
                    ...
                    <tbody>
                        <?php $i=1; foreach($result_httt as $items):?>
                        <tr>
                            <td><?php echo $i;?></td>
                            <td><?php echo $items['id'];?></td>
                            <td><?php echo $items['tenhinhthuc'];?></td>
                            <td><a class="btn-sua" href="hinhthucthanhtoan_sua.php?id=<?php echo $items['id']?>"><i class="fa-solid fa-pencil"></i></a></td>
                            <td><a class="btn-xoa" href="hinhthucthanhtoan_xoa.php?id=<?php echo $items['id']?>"><i class="fa-solid fa-trash-can"></i></a></td>
                        </tr>
                        <?php $i++; endforeach?>
                    </tbody>
                </table>      
