<?php
    ...
    $sql_pdkh = "SELECT pheduyetkhoahochoanthanh.id,khoahocgiaovien.id as idkh,theloaikhoahoc.tenkhoahoc,giaovien.hoten,giaovien.id as idgv,khoahocgiaovien.hocphi
                FROM pheduyetkhoahochoanthanh, khoahocgiaovien,giaovien,theloaikhoahoc
                WHERE pheduyetkhoahochoanthanh.idkhoahoc = khoahocgiaovien.id
                and pheduyetkhoahochoanthanh.idgiaovien = giaovien.id
                and khoahocgiaovien.giaovienid = giaovien.id
                and khoahocgiaovien.theloaikhoahocid = theloaikhoahoc.id";
    $stmt_pdkh = $conn->prepare($sql_pdkh);
    $query_pdkh  = $stmt_pdkh->execute();
    $result_pdkh  = array();
    while($row_pdkh = $stmt_pdkh->fetch(PDO::FETCH_ASSOC)){
        $result_pdkh[] = $row_pdkh;
    }

    $sql_sl = "SELECT count(id) as dem FROM pheduyetkhoahochoanthanh";
...
<body>
    <div id="content">
        <div id="admin__sidebar">
         ...
         <div id="main_content">
            <div class="title">
                <h2>Phê duyệt khóa học hoàn thành (<?php echo $slusers?>)</h2>
            </div>
            <div class="chitiet_content">
                <table class="table table-inverse">
                    ...
                    <tbody>
                        <?php $i=1; foreach($result_pdkh as $items):?>
                        <tr>
                            <td><?php echo $i;?></td>
                            <td><?php echo $items['idkh'];?></td>
                            ...
                            <td><?php
                                ...
                                ?></td>
                            <td><a class="btn-sua" href="giaodich_khoahoc_dongy.php?id=<?php echo $items['id']?>&&idkh=<?php echo $items['idkh']?>&&idgv=<?php echo $items['idgv']?>"><i class="fa-solid fa-check"></i></a></td>
                            <td><a class="btn-xoa" href="giaodich_khoahoc_tuchoi.php?id=<?php echo $items['idkh']?>"><i class="fa-solid fa-trash-can"></i></a></td>
                        </tr>
                        <?php $i++; endforeach?>
                    </tbody>
                </table>      
