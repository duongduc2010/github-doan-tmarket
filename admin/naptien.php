<?php
    ...
    $sql_admin = "SELECT naptien.*,taikhoan.hoten FROM naptien,taikhoan WHERE naptien.idtaikhoan = taikhoan.id";
    $stmt_admin = $conn->prepare($sql_admin);
    $query_admin = $stmt_admin->execute();
    $result_admin = array();
    while($row_admin = $stmt_admin->fetch(PDO::FETCH_ASSOC)){
        $result_admin[] = $row_admin;
    }

    $sql_sl = "SELECT count(id) as dem FROM naptien";
...
<body>
    <div id="content">
        <div id="admin__sidebar">
         ...
         <div id="main_content">
            <div class="title">
                <h2>Nạp tiền (<?php echo $slusers?>)</h2>
            </div>
            <div class="chitiet_content">
                <table class="table table-inverse">
                    ...
                    <tbody>
                        <?php $i=1; foreach($result_admin as $items):?>
                        <tr>
                            <td><?php echo $i;?></td>
                            <td><?php echo $items['hoten'];?></td>
                            <td><?php echo $items['sotien'];?></td>
                            <td><?php echo $items['thoigian'];?></td>
                            <td><a class="btn-sua" href="naptien_dongy.php?id=<?php echo $items['id']?>"><i class="fa-solid fa-check"></i></a></td>
                            <td><a class="btn-xoa" href="naptien_tuchoi.php?id=<?php echo $items['id']?>"><i class="fa-solid fa-trash-can"></i></a></td>
                        </tr>
                        <?php $i++; endforeach?>
                    </tbody>
                </table>      
