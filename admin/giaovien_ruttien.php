<?php
    ...
    $sql_rt = "SELECT ruttien.*, giaovien.id as idgv,giaovien.hoten FROM ruttien,giaovien
                WHERE ruttien.idgiaovien = giaovien.id";
    $stmt_rt = $conn->prepare($sql_rt);
    $query_rt  = $stmt_rt->execute();
    $result_rt  = array();
    while($row_rt = $stmt_rt->fetch(PDO::FETCH_ASSOC)){
        $result_rt[] = $row_rt;
    }

    $sql_sl = "SELECT count(id) as dem FROM ruttien";
...
<body>
    <div id="content">
        <div id="admin__sidebar">
         ...
         <div id="main_content">
            <div class="title">
                <h2>Danh sách yêu cầu rút tiền (<?php echo $slusers?>)</h2>
            </div>
            <div class="chitiet_content">
                ...
                <table class="table table-inverse">
                    ...
                    <tbody>
                        <?php $i=1; foreach($result_rt as $items):?>
                        <tr>
                            <td><?php echo $i;?></td>
                            <td><?php echo $items['id'];?></td>
                            <td><?php echo $items['hoten'];?></td>
                            <td><?php echo $items['tennganhang'];?></td>
                            <td><?php echo $items['sotaikhoan'];?></td>
                            <td><?php
                                $sotien = $items['sotien'];
                            ...
                            <td><?php 
                                ...
                                ?></td>
                            <td><a class="btn-sua" href="giaovien_ruttien_dongy.php?id=<?php echo $items['id']?>&&idgv=<?php echo $items['idgv']?>"><i class="fa-solid fa-check"></i></a></td>
                            <td><a class="btn-xoa" href="giaovien_ruttien_tuchoi.php?id=<?php echo $items['id']?>&&idgv=<?php echo $items['idgv']?>"><i class="fa-solid fa-trash-can"></i></a></td>
                        </tr>
                        <?php $i++; endforeach?>
                    </tbody>
                </table>      
