<?php
    ...
    $sql_users = "SELECT * FROM taikhoan";
    $stmt_users = $conn->prepare($sql_users);
    $query_users = $stmt_users->execute();
    $result_users = array();
    while($row_users = $stmt_users->fetch(PDO::FETCH_ASSOC)){
        $result_users[] = $row_users;
    }

    $sql_sl = "SELECT count(id) as dem FROM taikhoan";
...
<body>
    <div id="content">
        <div id="admin__sidebar">
         ...
         <div id="main_content">
            <div class="title">
                <h2>Người dùng mới (<?php echo $slusers?>)</h2>
            </div>
            <div  class="chitiet_content">
                <table class="table table-inverse">
                    ...
                    <tbody>
                        <?php $i=1; foreach($result_users as $items):?>
                        <tr>
                            <td><?php echo $i;?></td>
                            ...
                            <td><?php echo $items['phanquyen'];?></td>
                            <td><a class="btn-xoa" href="nguoidungmoi_xoa.php?id=<?php echo $items['id']?>"><i class="fa-solid fa-trash-can"></i></a></td>
                        </tr>
                        <?php $i++; endforeach?>
                    </tbody>
                </table>      
