<?php
    ...
    $sql_pdgv = "SELECT pheduyetgiaovien.*, theloaigiaovien.tentheloai
                FROM pheduyetgiaovien,theloaigiaovien
                WHERE  pheduyetgiaovien.theloaigiaovien = theloaigiaovien.id;";
    $stmt_pdgv = $conn->prepare($sql_pdgv);
    $query_pdgv = $stmt_pdgv->execute();
    $result_pdgv = array();
    while($row_pdgv = $stmt_pdgv->fetch(PDO::FETCH_ASSOC)){
        $result_pdgv[] = $row_pdgv;
    }

    $sql_sl = "SELECT count(id) as dem FROM pheduyetgiaovien";
...
<body>
    <div id="content">
        <div id="admin__sidebar">
         ...
         <div id="main_content">
            <div class="title">
                <h2>Phê duyệt giáo viên (<?php echo $slusers?>)</h2>
            </div>
                ...
                <table class="table table-inverse">
                    <thead>
                        <tr>
                            ...
                            <th>Email</th>
                            <th>Số điện thoại</th>
                            <th>Thể loại giáo viên</th>
                            <th>Hành động</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php $i=1; foreach($result_pdgv as $items):?>
                        <tr>
                            <td><?php echo $i;?></td>
                            ...
                            <td><?php echo $items['tentheloai'];?></td>
                            <td><a class="btn-thongtinchitiet" href="pheduyetgiaovien_chitiet.php?idpdgv=<?php echo $items['id']?>"><i class="fa-solid fa-eye"></i></a></td>
                            <td><a class="btn-sua" href="pheduyetgiaovien_dongy.php?idpdgv=<?php echo $items['id']?>"><i class="fa-solid fa-check"></i></a></td>
                            <td><a class="btn-xoa" href="pheduyetgiaovien_xoa.php?idpdgv=<?php echo $items['id']?>"><i class="fa-solid fa-trash-can"></i></a></td>
                        </tr>
                        <?php $i++; endforeach?>
                    </tbody>
                </table>      
