<?php
	...
	if(isset($_POST['submit_them'])){
		if(!empty($_POST['cauhoi']) && !empty($_POST['a']) && !empty($_POST['b']) && !empty($_POST['c']) && !empty($_POST['d']) && isset($_POST['chon'])){
			...
			$sql_sua = "UPDATE chitietbaikiemtra SET tencauhoi = '$cauhoi',dapana='$a',dapanb='$b',dapanc='$c',dapand='$d',dapandung='$dapandung' WHERE idbkt = '$idbkt' and STT='$idcauhoi'";
	...
	if(isset($_POST['submit_hoantat'])){
		header("location:chitietkhoahoc.php?id=$id_teacher&&idgv=$idgv&&idkh=$idkh");
	}
...
<body>
	...
	<div id="content">
			...
			<div class="warning">
				...
				<h5>Nhấn "Sửa" sau khi đã sửa đầy đủ câu hỏi của bài tập</h5>
				<h5>Lưu ý: Phải điền đầy đủ thông tin câu hỏi và tích chọn đáp án đúng thì câu hỏi mới được sửa lại vào bài tập</h5>
			</div>
			<form method="post">
				...
				<!-- <button class="btnAdd">Thêm câu hỏi</button> -->
					<input type="submit" name="submit_them" class="btn" value="Sửa"/>
				</div>
			</form>
	</div>
