<?php
	...
	$idbkt = $_GET['idbkt'];
	$idkh = $_GET['idkh'];
	$sql_bt = "SELECT baikiemtra.tenbaitap FROM baikiemtra WHERE id = '$idbkt'";
	...
	foreach ($result_bt as $items) {
		$tenbt = $items['tenbaitap'];
	}

	
	$pheduyetthanhcong = "Đang phê duyệt";
	$sql_hv = "SELECT DISTINCT taikhoan.id, taikhoan.hoten,taikhoan.email,taikhoan.sodienthoai
				FROM ketqualambaitap,taikhoan,baikiemtra
				WHERE ketqualambaitap.idbaikiemtra = baikiemtra.id
				and ketqualambaitap.idhocvien = taikhoan.id
				and ketqualambaitap.idbaikiemtra = '$idbkt'";
...
<body>
	...
	<div id="content">
		<h1>Tên bài tập: <?php echo $tenbt?></h1>
				...
				<tbody>
					<?php $i=1; foreach ($result_hv as $items): ?>
						<tr>
							...
							<td><a href="lambaitap.php?id=<?php echo $id_teacher?>&&idhv=<?php echo $items['id']?>&&idkh=<?php echo $idkh?>">Chi tiết</a></td>
						</tr>
					<?php $i++; endforeach ?>
				</tbody>
			</table>
