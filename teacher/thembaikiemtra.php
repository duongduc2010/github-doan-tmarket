<?php
	...
	if(isset($_POST['submit'])){
		if(!empty($_POST['tenbai'])){
			$tenbai = $_POST['tenbai'];
			$sql_tenbaikt = "INSERT INTO baikiemtra(tenbaitap,idkhoahoc) VALUES('$tenbai','$idkh')";
			$stmt_tenbaikt = $conn->prepare($sql_tenbaikt);
			$query_tenbaikt = $stmt_tenbaikt->execute();
			if($query_tenbaikt){
				//lay id bài kiểm tra vừa tạo
				$sql_baikt = "SELECT * FROM baikiemtra WHERE idkhoahoc='$idkh'";
				$stmt_baikt = $conn->prepare($sql_baikt);
				$query_baikt = $stmt_baikt->execute();
				while($row_baikt=$stmt_baikt->fetch(PDO::FETCH_ASSOC)){
					$result_baikt[] = $row_baikt;
				}

				foreach ($result_baikt as $items) {
					$idbkt = $items['id'];
				}
				header("location:themcauhoi.php?id=$id_teacher&&idgv=$idgv&&idkh=$idkh&&idbkt=$idbkt");
			}
			else{
				echo "Phải đặt tên cho bài kiểm tra";
			}
...
<body>
	...
	<div id="content">
			...
			<div class="warning">
				<h5>Chú ý: Đặt tên cho bài tập, sau đó chọn "Đăng bài tập" để thêm câu hỏi cho bài tập</h5><br>
			...
			<form method="post">
				<div class="container-box">
					<div class="name-question">
						...
						<input required name="tenbai" type="text" placeholder="Tên của bài tập này">
					</div>
					<!-- <div class="box">
						<div class="box-child">
							<label for="">Câu hỏi</label> -->
							<!-- <input type="text" > -->
							<!-- <textarea class="hi" name="cau1" id="" cols="50" rows="2" placeholder="Nội dung câu hỏi"></textarea>
						</div> -->
						<!-- <div class="box-rep">
							<input type="radio" name="a" id="" value="A">
							<input name="a1" type="text" placeholder="Nội dung câu trả lời"><br>
							<input type="radio" name="a" id="" value="B">
							<input name="b1" type="text" placeholder="Nội dung câu trả lời"><br>
							<input type="radio" name="a" id="" value="C">
							<input name="c1" type="text" placeholder="Nội dung câu trả lời"><br>
							<input type="radio" name="a" id="" value="D">
							<input name="d1" type="text" placeholder="Nội dung câu trả lời"><br>
						</div> -->
					<!-- </div> -->
				</div>
				...
				<!-- <button class="btnAdd">Thêm câu hỏi</button> -->
					<input type="submit" name="submit" class="btn" value="Đăng bài tập"/>
