<?php
	...
	$pageNext = $pageNum+1;
	//Danh sách ý kiến
	$sql_dsyk = "SELECT theloaikhoahoc.tenkhoahoc,ykiendanhgia.*
				FROM ykiendanhgia,theloaikhoahoc,khoahocgiaovien
				WHERE ykiendanhgia.idhocvien = '$iduser'
				and ykiendanhgia.idkhoahoc = khoahocgiaovien.id
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="style/lsdg.css">
	...
	<div id="content">
		...
		<div class="dsgv">
			...
			<div class="product__pagination">
	            <?php if($pageNum>1){?>
					<a href="lichsudanhgia.php?pageNum=<?id=<?php echo $iduser?>&&php echo $pagePrev?>"> < </a>
				<?php }?>
				<?php for($i=$from;$i<=$to;$i++){?>
					<?php if($i==$pageNum){?>
						<a class="active" href="lichsudanhgia.php?id=<?php echo $iduser?>&&pageNum=<?php echo $i?>"><?php echo $i?></a>
					<?php } else {?>
						<a href="lichsudanhgia.php?id=<?php echo $iduser?>&&pageNum=<?php echo $i?>"><?php echo $i?></a>
	        		<?php }?>
	        	<?php }?>
	        	<?php if($pageNum<$tongSoTrang){?>
					<a href="lichsudanhgia.php?id=<?php echo $iduser?>&&pageNum=<?php echo $pageNext?>"> > </a>
				<?php }?>
	        </div>
