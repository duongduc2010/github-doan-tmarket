<?php
	...
	if(!empty($_POST['submit'])){
		if(isset($_POST['matkhaucu']) && isset($_POST['matkhaumoi']) && isset($_POST['nhaplaimatkhaumoi'])){
			...
			$sql_taikhoan = "SELECT taikhoan.matkhau FROM taikhoan WHERE id = '$iduser'";
			...
			if(($mkmoi == $mkmoi_again) && (md5($mkcu) == $matkhau_taikhoan)){
				$mkm = md5($mkmoi);
				$sql_matkhau = "UPDATE taikhoan SET matkhau ='$mkm' WHERE id ='$iduser'";
				$stmt_matkhau = $conn->prepare($sql_matkhau);
				$query_matkhau = $stmt_matkhau->execute();
				if($query_matkhau){
					header("location:thaydoimatkhau.php?id=$iduser");
				}
				else{
					echo "Lỗi";
				}
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="style/thaydoimk.css">
	...
	<div id="content">
				...
				<div class="chitiet">
					<form method="post">
						<fieldset class="form-group">
							...
							<input name="matkhaucu" type="password" class="form-control" id="mkcu" placeholder="Mật khẩu cũ">
						</fieldset>
						<fieldset class="form-group">
							<label for="mkmoi">Mật khẩu mới</label>
							<input name="matkhaumoi" type="password" class="form-control" id="mkmoi" placeholder="Mật khẩu mới">
						</fieldset>
						<fieldset class="form-group">
							<label for="nhaplaimkmoi">Nhập lại mật khẩu mới</label>
							<input name="nhaplaimatkhaumoi" type="password" class="form-control" id="nhaplaimkmoi" placeholder="Nhập lại mật khẩu mới"></input>
						</fieldset>
						<fieldset class="form-group">
							<input name=" submit" class="form-control luu" type="submit" value="Lưu lại">
						</fieldset>
					</form>
				</div>
			</div>
		...
		<div class="dsgv">
			...
			<div class="product__pagination">
	            <?php if($pageNum>1){?>
					<a href="thaydoimatkhau.php?pageNum=<?id=<?php echo $iduser?>&&php echo $pagePrev?>"> < </a>
				<?php }?>
				<?php for($i=$from;$i<=$to;$i++){?>
					<?php if($i==$pageNum){?>
						<a class="active" href="thaydoimatkhau.php?id=<?php echo $iduser?>&&pageNum=<?php echo $i?>"><?php echo $i?></a>
					<?php } else {?>
						<a href="thaydoimatkhau.php?id=<?php echo $iduser?>&&pageNum=<?php echo $i?>"><?php echo $i?></a>
	        		<?php }?>
	        	<?php }?>
	        	<?php if($pageNum<$tongSoTrang){?>
					<a href="thaydoimatkhau.php?id=<?php echo $iduser?>&&pageNum=<?php echo $pageNext?>"> > </a>
				<?php }?>
	        </div>
