<?php
	...
	//Lịch sử giao dịch
	$sql_lsgd = "SELECT * FROM lsgiaodich WHERE idtaikhoan='$iduser'";
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="style/vicuatoi.css">
...
<body>
	...
	<div id="content">
		...
		<div class="noidungchinh">
			...
			<div class="cot hai">
				<div class="button">
					<button id="defaultOpen" class="tablinks" onclick="thongkeFunction(event,'tmarket')">Ví Tmarket</button>
					<!-- <button  class="tablinks" onclick="thongkeFunction(event,'khoahoc')">Ví khóa học</button> -->
				</div>
				<div id="tmarket" class="tabcontent">
						...
						<tbody>
							<?php $i=1; foreach ($result_lsgd as $items): ?>
								<tr>
									<td><?php echo $i?></td>
									<td><?php 
									$ngaythang = $items['ngaythang'];
									echo date("d-m-Y",strtotime($ngaythang));
									?></td>
									<td><?php echo $items['mota']?></td>
									<td><?php echo number_format($items['biendong'])?></td>
									<td><?php echo number_format($items['tienconlai'])?></td>
								</tr>
							<?php $i++; endforeach ?>
						</tbody>
				...
				<!-- <div id="khoahoc" class="tabcontent">
					<table class="table table-inverse">
						<thead>
							<tr>
								<th>STT</th>
		...
		<div class="dsgv">
			...
			<div class="product__pagination">
	            <?php if($pageNum>1){?>
					<a href="vicuatoi.php?pageNum=<?id=<?php echo $iduser?>&&php echo $pagePrev?>"> < </a>
				<?php }?>
				<?php for($i=$from;$i<=$to;$i++){?>
					<?php if($i==$pageNum){?>
						<a class="active" href="vicuatoi.php?id=<?php echo $iduser?>&&pageNum=<?php echo $i?>"><?php echo $i?></a>
					<?php } else {?>
						<a href="vicuatoi.php?id=<?php echo $iduser?>&&pageNum=<?php echo $i?>"><?php echo $i?></a>
	        		<?php }?>
	        	<?php }?>
	        	<?php if($pageNum<$tongSoTrang){?>
					<a href="vicuatoi.php?id=<?php echo $iduser?>&&pageNum=<?php echo $pageNext?>"> > </a>
				<?php }?>
	        </div>
	...
	<script>
		...
		function thongkeFunction(evt,detailName){
